...
import {W,H,TILE,BUILDINGS,placement,cost,place,markClearing,plantTree,removePath,movePlacement,moveBuilding} from './world.js';
import {ITEMS,RECIPES,RESEARCH,EXTRA_BUILDINGS,stock,connectStorage,isHub} from './industry.js';
const $=id=>document.getElementById(id);
const esc=v=>String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const icons={wood:'🪵',stone:'🪨',plank:'▰',brick:'▤',ore:'◆',coal:'●',iron:'▱',gear:'⚙',star:'✦'};
...
export function installQoL(api){
  ...
  panel.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
    if(b.dataset.land){resource=null;land=b.dataset.land;landKey='';renderLand();}
    if(b.dataset.brush!==undefined){brush=Number(b.dataset.brush);landKey='';renderLand();}
  ...
  function reset(){site=null;moving=null;land=null;resource=null;buildKey='';landKey='';panel.hidden=true;$('landBtn').classList.remove('active');}
  ...
  function renderLand(){
    ...
    const key=[land,brush,resource,!!n?.clear,n?.amount,queued,!!undo].join(':');if(key===landKey)return;landKey=key;panel.hidden=false;
    ...
    if(land==='clear'||land==='keep')panel.innerHTML+='<div class="brushrow" aria-label="Brush size">'+[[0,'1 tile'],[1,'3 × 3'],[2,'5 × 5']].map(([k,t])=>`<button class="secondary ${brush===k?'active':''}" data-brush="${k}" aria-pressed="${brush===k}">${t}</button>`).join('')+'</div>';
    panel.innerHTML+=`<p class="taskcopy">${land==='plant'?'Tap empty grass to plant a renewable tree.':land==='path'?'Tap a footpath to remove it and recover its stone.':land==='keep'?'Tap marked patches to keep them.':land==='clear'?'Tap a patch. Folk gather its materials, then remove the roots. Drag to pan.':'Clear land for building, or keep it for resources.'}</p><div class="queuecount">${queued?`${queued} patches waiting for little hands`:'No clearing jobs waiting'}</div>${undo?'<button class="secondary wide" id="landUndo">Undo unfinished marks</button>':''}`;
  }
  function renderBuild(){
    ...
    $('buildText').textContent=RECIPES[type]?`${Object.entries(RECIPES[type].input).map(([k,n])=>`${n} ${ITEMS[k].name.toLowerCase()}`).join(' + ')||'Wind'} → ${RECIPES[type].amount} ${ITEMS[RECIPES[type].output].name.toLowerCase()}`:({hut:'A home brings a new friend.',path:'Tap to lay faster walking routes.',depot:'Shorter trips. Shared village storage.',windmill:'Powers nearby workshops. The circle shows its reach.',lumber:'Helps nearby folk gather wood faster.',quarry:'Helps nearby folk gather stones faster.',garden:'A little place for flowers and quiet breaks.',workshop:'Better tools for all your folk.'}[type]||'Choose a place on the map.');
    ...
    if(type==='path'){html+='<p class="taskcopy">Tap grass to place one tile. Use Land → Lift path to remove it.</p>';}
      ...
      if(nodes.length)html+=`<button class="secondary wide" id="clearSite" ${allMarked?'disabled':''}>${allMarked?'Your folk are clearing this site':'Clear this site first'}</button>`;
  ...
  function performGoal(){if(!goalTask)return;const t=goalTask;if(t.build)api.chooseBuild(t.build);else if(t.inspect)api.inspectBuilding(t.inspect);else if(t.book)api.openBook(t.book);else if(t.land)enterLand();}
  function nextTask(){
  ...
  function sync(){
    ...
    const active=!$('inspector').hidden||!$('buildHint').hidden||!$('routeHint').hidden||!panel.hidden;
  ...
  function update(){
    ...
    goalTask=nextTask();$('goalTag').textContent='A little next step';$('goalTitle').textContent=goalTask.title;$('goalText').textContent=goalTask.text;$('goalProgress').style.width=goalTask.progress*100+'%';$('goalAction').textContent=goalTask.button;
  ...
  function draw(c){
    ...
    const h=api.getView().hover;if(land&&h){const r=land==='clear'||land==='keep'?brush:0;c.strokeStyle='#fff6cf';c.lineWidth=1;c.strokeRect((h.x-r)*TILE,(h.y-r)*TILE,(r*2+1)*TILE,(r*2+1)*TILE);}c.restore();
  }
  function modalChanged(){sync();$('toast').hidden=true;}
  layout();return {update,sync,reset,buildChanged,preview,startMove,testSite,mapTap,draw,modalChanged,get hasSite(){return !!site;},get site(){return site;}};
}
