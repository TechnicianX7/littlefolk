...
export function installBloomUI(api){
  ...
  function renderBoard(){
    ...
    if(tab==='supper'){
      ...
      html+='<div class="recipebig">🥕 Vegetables → 🍲 Cookhouse → 🏡 Home</div><div class="smallactions">'+['vegetable','kitchen','orchard','fishery','garden'].map(t=>`<button class="secondary" data-guide="${t}">${BUILDINGS[t].short}</button>`).join('')+'</div><p class="footnote">Food producers work without windmills. A side-dish portion serves three residents. Flowers add a small spirit bonus. Closing the game freezes the day; there is no absence penalty.</p>';
    }
    if(tab==='chains'){
